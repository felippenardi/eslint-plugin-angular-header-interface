      ...
      return {
        ...
        MemberExpression: function(node) {
          ...
          if(isAngularModuleMethod &&
            ...
            if (angularClass !== firstVariableName) {
              context.report(
                firstVariable,
                'Use "' +
                angularClass +
          ...
          if(isAngularModuleMethod &&
            isOneOfAngularMainClasses) {
            var returnVariable = _.filter(node.parent.arguments[1].body.body, function(element) {
        ...
        ReturnStatement: function(node) {
          ...
          if (isTheInterfaceReturn) {
            ...
            if(returnVariableName !== angularClass) {
              context.report(
                returnVariable,
                'Return "' +
                angularClass +
            ...
            var expressionStatementsBeforeReturn =
              ...
              _.map(expressionStatementsBeforeReturn, function(expression) {
                if(_.get(expression, 'expression.callee.name') !== 'activate') {
                  context.report(
                    expression,
                    'Only activate() can before interface object is returned.'
                  );
                }
              });
